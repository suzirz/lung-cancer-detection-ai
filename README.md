...
### Option A: 1-Click Interactive Web Demo (No Local Setup)
👉 **[Open In Google Colab](https://colab.research.google.com/github/suzirz/lung-cancer-detection-ai/blob/main/notebooks/train_colab_t4.ipynb)** — Run all cells in Colab to train or evaluate on a free cloud GPU.
...
```
Open `http://localhost:8501` in your browser. Select any pre-loaded clinical CT slice (Benign, Malignant, Normal) or upload your own, and inspect the real-time diagnostic probabilities and Grad-CAM heatmap in ~150 ms.

### Non-Technical Overview (TL;DR)
PulmoScan is an AI triage assistant for chest CT scans. It analyzes an axial lung slice, predicts whether it is **Normal**, **Benign**, or **Malignant**, and overlays a visual heat map (Grad-CAM) showing the exact parenchymal region driving the decision so clinicians can immediately audit *where* and *why* the model made its assessment.
...
- [Problem Context: Why This Matters](#problem-context-why-this-matters)
- [Clinical Decision Support & Triage Workflow](#clinical-decision-support--triage-workflow)
- [Addressing Model Metrics & The 100% Accuracy Pitfall](#addressing-model-metrics--the-100-accuracy-pitfall)
...
- [5-Fold Cross-Validation & Architecture Ablation](#5-fold-cross-validation--architecture-ablation)
- [Mathematical Formulation & Radiomics Taxonomy](#mathematical-formulation--radiomics-taxonomy)
...
- [Multimodal Architecture & Radiomics Fusion](#multimodal-architecture--radiomics-fusion)
- [Codebase Design & Module Hierarchy](#codebase-design--module-hierarchy)
- [Developer Guide: Python API & CLI Workflows](#developer-guide-python-api--cli-workflows)
...
- [Hospital Deployment & DICOM PACS Integration](#hospital-deployment--dicom-pacs-integration)
- [Clinical Failure Modes & Differential Diagnosis](#clinical-failure-modes--differential-diagnosis)
...
- [Community, Peer Review & Open Clinical Feedback](#community-peer-review--open-clinical-feedback)
- [Academic Bibliography & Citations](#academic-bibliography--citations)
- [License](#license)
...
## Clinical Decision Support & Triage Workflow

In high-volume radiology departments, reading queues routinely back up by several days. PulmoScan is designed as an automated second-reader and worklist prioritization filter. The operational triage protocol follows four sequential stages:
...
Axial chest CT slices are converted from DICOM attenuation values (Hounsfield Units) or raw pixel formats, rescaled to $224 \times 224$, and normalized to standard distribution channels.

### 2. Dual-Threshold Risk Stratification
Standard multi-class models use a simple $\text{argmax}$ rule, which implicitly sets an operating threshold of $0.33$ for three classes. In clinical oncology, false negatives are far more dangerous than false positives. PulmoScan decouples the decision boundary:
- **Operating Malignancy Threshold ($\tau_{\text{mal}} = 0.35$)**: Any slice with $P(\text{Malignant}) \ge 0.35$ is escalated immediately to the top of the specialist reading queue, regardless of whether another class has a higher raw score.
- **Indeterminate Zone ($0.15 \le P(\text{Malignant}) < 0.35$)**: Scans falling into this margin trigger an "Indeterminate" alert requiring secondary review.
- **Routine Normal/Benign ($P(\text{Malignant}) < 0.15$)**: Processed through regular reporting schedules.

### 3. Spatial Grounding & Radiomics Verification
When an alert fires, the radiologist does not receive a blind label. The interface presents:
1. **Spatial heatmaps (Grad-CAM)** highlighting the exact convolutional activations driving the decision.
2. **Quantitative radiomics table** detailing edge sharpness ($\sigma^2_{\text{Laplace}}$), tissue heterogeneity (Shannon entropy), and density moments to verify whether the attention corresponds to true spiculation, cavitation, or pleural traction.

### 4. Structured Reporting Export
Predictions and saliency overlays map directly into standard DICOM Structured Reporting (SR) or HL7 FHIR Observation formats for automated PACS integration.

---

## Addressing Model Metrics & The 100% Accuracy Pitfall

In medical machine learning, reported metrics of **100% accuracy** should be treated with extreme skepticism. In computer vision benchmarks, perfect separation is almost always caused by **data leakage** or **evaluation on identical augmented copies**.

### 1. The Naive Split Illusion
In early prototype iterations and across many public notebooks trained on IQ-OTHNCCD, standard random splitting (`train_test_split`) operated at the individual image level. Because the dataset contains ~10 augmented variants per original CT scan (named `{Class} case (X)(Y).jpg`), rotating and flipping copies of patient `X` were scattered across both train and test splits. The network memorized patient-specific anatomical quirks, yielding an artificial 100.00% accuracy.

### 2. Group-Aware Partitioning (True Patient Isolation)
PulmoScan eliminates this flaw via group-aware stratified splitting (`src/preprocessing/split.py`). All augmented variants of any single patient scan are strictly assigned to either Train, Validation, OR Test — never split across sets. With complete patient isolation on held-out test scans:
- Overall test accuracy drops to **98.47%** (1,733 of 1,760 scans correct).
- **Malignant recall** remains **100.00%** (624 of 624 malignant scans correctly flagged; zero false negatives).
- **Normal parenchyma recall** is **96.24%** (26 normal scans misclassified as benign due to vascular bundling and apical density).
- **Benign nodule recall** is **99.77%** (1 benign lesion misclassified as malignant due to irregular margin).

### 3. Patient-Level Aggregation (Majority Voting)
Because the IQ-OTHNCCD dataset provides augmented variants, evaluating individual slices still means testing ~10 variants per test patient. To evaluate genuine patient-level accuracy, predictions across all slices belonging to each patient group $g$ are aggregated via majority voting:

$$\hat{y}_g = \text{mode}\left(\left\{\hat{y}_i : i \in \mathcal{S}_g\right\}\right)$$

Across all 329 held-out patient groups in the test set:
- **Patient-Level Diagnostic Accuracy**: 98.78% (325 / 329 patients correctly diagnosed).
- **Patient-Level Malignant Sensitivity**: 100.00% (117 / 117 malignant patients identified).
...
In oncology machine learning, **raw accuracy is a dangerous vanity metric**. An algorithm that correctly identifies normal scans and large tumors can post a 95%+ accuracy while missing subtle early-stage malignancies or suffering from severe class-specific dropoffs.

PulmoScan establishes an explicit metrics hierarchy based on clinical utility:

| Metric | Priority Level | Clinical Rationale | PulmoScan Score (Held-Out Test) |
|---|---|---|---|
| **Malignant Recall (Sensitivity)** | 🚨 **Rank 1 (Critical)** | A missed malignant nodule (false negative) is catastrophic — patients leave undiagnosed. | **100.00%** (624 / 624 identified) |
| **Macro F1-Score** | ⚖️ **Rank 2 (Balanced)** | Unweighted harmonic mean of precision and recall across all classes, penalizing any class failure equally. | **98.41%** (CNN) · **98.83%** (Hybrid) |
| **Confusion Matrix Topology** | 🔍 **Rank 3 (Auditing)** | Exposes exact off-diagonal error patterns (e.g. vascular branching misdiagnosed as benign). | **26 normal → benign errors** |
| **5-Fold Cross-Validation** | 🛡️ **Rank 4 (Stability)** | Prevents reporting a single lucky split; tests generalization across 5 independent patient folds. | **Macro F1: 98.48% ± 0.20%** |
| **Overall Accuracy** | 📊 **Rank 5 (Baseline)** | General proportion of correct predictions across all 1,760 held-out test scans. | **98.47%** (1,733 / 1,760 correct) |
...
> Notice: The hybrid tree classifiers increase **Macro F1 by +0.42%** and **Macro Recall by +0.36%** over the CNN softmax head, specifically because balanced decision trees adjust split boundaries to protect minority and borderline nodule classes.
...
1. **Zero Missed Malignancies**: Out of 624 malignant CT scans, exactly **0 were misclassified as normal or benign**. Every single malignancy triggered a positive referral.
2. **The 26 Normal-to-Benign False Positives**: All 26 misclassifications in the normal class were predicted as benign opacities (never malignant). Radiologically, these correspond to dense bronchovascular bundles, pulmonary vessel branching in the hilar region, or pleural thickening that resembles small, well-circumscribed benign nodules.
3. **The 1 Benign-to-Malignant False Positive**: Exactly 1 benign nodule was classified as malignant due to an irregular margin simulating spiculation. Clinically, this is a safe failure mode: it results in an extra biopsy or follow-up CT rather than a missed tumor.
...
## 5-Fold Cross-Validation & Architecture Ablation

To verify that the high test metrics are not an artifact of an unusually fortunate split partition, PulmoScan includes an automated 5-fold group-stratified cross-validation protocol (`src/evaluation/cross_validation.py`).
...
### Hardware Profiling & Inference Latency

Measurements performed with single-image forward inference (batch size = 1) averaged over 500 iterations:
...
## Mathematical Formulation & Radiomics Taxonomy

PulmoScan pairs deep convolutional representations with quantitative tissue statistics.

### 1. Convolutional Representation Learning
...
8. **Contrast Laplacian Variance ($\sigma_{\text{Laplace}}^2$)**:
   ...
   *Quantifies edge sharpness; malignant nodules with irregular, spiculated borders exhibit high Laplacian variance, whereas ground-glass benign opacities exhibit smooth, low variance.*
...
13. **Interquartile Range ($IQR$)**: $IQR = P_{75} - P_{25}$, providing a dispersion metric insensitive to outlier artifacts.

$$f_{\text{radiomics}} = [\mu, \sigma, \sigma^2, \gamma_1, \gamma_2, E, H, \sigma_{\text{Laplace}}^2, \mu_G, \sigma_G, P_{25}, P_{75}, IQR]^T \in \mathbb{R}^{13}$$

### 3. Multimodal Concatenation & Tree Objective
...
where $N$ is total training images, $K = 3$ is the number of diagnostic classes, and $N_c$ is the frequency of class $c$. This formulation applies a larger penalty to misclassifications in underrepresented classes.
...
where $Z = H \times W$ is the spatial area of the activation map. The class-discriminative localization map is obtained through rectified linear combination:

$$L_{\text{Grad-CAM}}^c = \text{ReLU}\left(\sum_k \alpha_k^c A^k\right)$$

The $\text{ReLU}$ function retains features that positively contribute to the target class $c$, suppressing activations that support competing diagnoses. The resulting heatmap is bilinearly interpolated back to the original slice dimensions ($224 \times 224$) and blended via weighted alpha compositing:
...
PulmoScan has currently been evaluated only on the single-center IQ-OTHNCCD dataset (Iraq-Oncology Teaching Hospital). In accordance with the TRIPOD-AI and CLAIM medical imaging reporting guidelines, single-center benchmarks cannot substantiate claims of clinical generalizability.

### The LIDC-IDRI Benchmark Challenge
To rigorously evaluate generalizability across scanner manufacturers, slice thicknesses, and patient demographics, we have developed a validation roadmap using the **LIDC-IDRI (Lung Image Database Consortium)** dataset:
- **Cohort**: 1,018 patients across 7 academic centers.
- **Annotations**: 4 experienced thoracic radiologists with independent nodule boundary contours and 1–5 malignancy ratings.
- **DICOM Volumetric Data**: Unaugmented, full-depth 3D CT volumes containing real Hounsfield Units (-1000 HU to +1000 HU).
...
## Codebase Design & Module Hierarchy

PulmoScan follows deep module design principles: modules expose simple, stable interfaces while hiding complex internal workflows, device transfers, and error-handling routines.

```text
src/
├── preprocessing/
│   ├── dataset.py            ← Deep: Image scanning, patient group parsing, PyTorch Dataset
│   └── split.py              ← Deep: Group-aware stratified splitting, leak prevention assertions
├── models/
│   ├── cnn_extractor.py      ← Deep: Pretrained backbone, surgical head separation, feature/logit dual mode
│   ├── ml_classifier.py      ← Deep: Scikit-learn & XGBoost wrappers, balanced weighting, joblib persistence
│   └── radiomics_extractor.py← Deep: Intensity moments, Shannon entropy, OpenCV gradient morphology
├── explainability/
│   ├── gradcam.py            ← Deep: Forward/backward PyTorch hooks, ReLU pooling, colormap blending
├── evaluation/
│   ├── metrics.py            ← Deep: Multi-class recall, precision, confusion matrix, ROC-AUC curves
│   └── cross_validation.py   ← Deep: 5-fold group-stratified evaluation loop
└── training/
    ├── train.py              ← Orchestrator: Epoch training loop, PyTorch AMP FP16, checkpointing
    └── run_colab.py          ← Automation: 1-click cloud execution script for Kaggle/Colab T4
```

### Module Interface Summary:

| Module | Primary Interface | Hidden Implementation Details | Seams & Decoupling |
|---|---|---|---|
| `split.py` | `split_and_save(...)` | Group regex parsing, stratified grouping, cross-split zero-overlap verification | Pure file-based JSON output; no PyTorch dependency |
| `dataset.py` | `get_dataloaders(...)` | PIL image decoding, RGB conversion, ImageNet normalization, PyTorch `DataLoader` setup | Separates disk IO from tensor batching |
...
| `ml_classifier.py` | `train_hybrid_classifier(...)`, `predict(...)` | Class-weight balancing, hyperparameter tuning grid-search, model serialization | Consumes NumPy arrays; completely independent of PyTorch |
| `gradcam.py` | `GradCAM.generate_heatmap(...)`, `overlay_heatmap(...)` | Backward hook registration, gradient tensor pooling, normalization bounds check | Attaches to any arbitrary PyTorch convolutional layer |

---

## Developer Guide: Python API & CLI Workflows

PulmoScan can be integrated programmatically into clinical research applications or executed directly from the terminal.
...
# 1. End-to-End Classification Logits
with torch.no_grad():
...
#### Extracting Quantitative Radiomics & Multimodal Fusion:
```python
...
|---|---|---|
| **Group-Aware Splitting** | `python -m src.preprocessing.split` | Scans dataset, checks patient groups, generates `data/splits/*.json` |
| **Model Training** | `python -m src.training.train --config configs/default.yaml` | Trains EfficientNet-B0 with FP16 mixed precision on GPU/CPU |
...
All pipeline hyperparameters are centralized in `configs/default.yaml`:
...
## Hospital Deployment & DICOM PACS Integration

PulmoScan can be integrated into hospital enterprise radiology networks as an on-premise, containerized inference worker.
...
A complete standalone container can be built and deployed without external internet access:
...
## Clinical Failure Modes & Differential Diagnosis
...
### 3. Juxta-Pleural & Sub-Pleural Lesions
...
### 4. Slice Thickness & Partial Volume Averaging
...
## Community, Peer Review & Open Clinical Feedback
